...
namespace ProductService.Products.AppServices.ProductService
{
    public class ProductService : IProductService
    {
        ...
        public async Task<Product> GetById(long id, CancellationToken cancellationToken = default)
        {
            ...
            if (product == null)
            {
                throw new InvalidOperationException($"Не удалось получить продукт с такми id - {id}");
            }
            return product;
        }
        ...
        public async Task Update(long id, string name, decimal price, CancellationToken cancellationToken = default)
        {
            ...
            if (product == null)
            {
                throw new InvalidOperationException($"Не удалось обновить продукт с таким id - {id}");
            }
        ...
        public async Task Delete(long id, CancellationToken cancellationToken = default)
        {
            ...
            if (product == null)
            {
                throw new InvalidOperationException($"Не удалось удалить продукт с таким id -{id}");
            }
